...
const parametroEmpresa01 = require("../parametros/parametro-empresa01.js");
  ...
  try {
    ...
    if (emp == null) {
      throw new erroDB.UserException("Regra de negócio", [
        { tabela: "EMPRESA", message: `Empresa Não Encontrada!` },
      ]);
    }
  } catch (error) {
    throw error;
...
exports.AtivarEmpresas = async function () {
  try {
    parametroEmpresa01.ativo = "S";
    const empresas = await this.getEmpresas(parametroEmpresa01);
    empresas.forEach((emp) => {
      blingSrv.setAgendamentoEmpresa(emp.id, true);
    });
  } catch (error) {
    throw error;
  }

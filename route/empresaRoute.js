...
router.get("/api/complementaempresa/:id_empresa", async function(req, res) {
    try {
        ...
        if ((response.deposito <= 0) && (response.categoria <= 0)) {
            res.status(409).json({ message: "Não Foi Possivel Complementar A Empresa!" });
        } else {
            res.status(200).json({ message: "Empresa Complementada Com Depósito e Categoria!", response });
        }
    } catch (error) {
        ...
        if (error.response) {
            ...
            if (error.name == 'MyExceptionDB') {
                ...
                res.status(500).json({ erro: 'BAK-END', tabela: 'Empresa', message: error.message });
            }
        }
    }
})
